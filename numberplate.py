...
def main():
    ...
    for counter in xrange(7 , 1 , -1):
        ...
        img=cv2.resize(cv2.imread('sample5.jpg') , ((counter + 2 ) * 100 , counter * 100 ))
    ...
    ###### For Pedestrian Crossing
        ...
        for contourWithData in allContoursWithData: 
            ...
            # if contourWithData.checkIfContourIsValid():
            ratio = float( intWidth ) / intHeight
            if ( ratio > 2  and ratio < 5 and ( contourWithData.fltArea > 200.0 and contourWithData.fltArea < 500.0 )):
            # if contourWithData.checkIfContourIsValid():
                # contourWithData=np.array(contourWithData)
                print contourWithData.fltArea , ratio 
